resource "azurerm_firewall" "main" {
  ...
  sku_name            = "AZFW_VNet"
  sku_tier            = "Standard"
  dns_servers         = local.firewall_allow_outbound ? var.dns_servers : []
...
resource "azurerm_route_table" "app" {
  name                = format("%s-rbc-app-rt", var.partner)
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
  tags                = var.tags
...
resource "azurerm_subnet_route_table_association" "gateway_pl" {
  subnet_id      = azurerm_subnet.gateway_pl.id
  route_table_id = azurerm_route_table.app.id
}

resource "azurerm_subnet_route_table_association" "fs" {
  subnet_id      = azurerm_subnet.fs.id
  route_table_id = azurerm_route_table.app.id
}
