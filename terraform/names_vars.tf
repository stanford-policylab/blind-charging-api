...
locals {
  container_app_env_name                = coalesce(var.container_app_env_name, lower(format("%s-rbc-env", var.partner)))
  container_app_name                    = coalesce(var.container_app_name, lower(format("%s-rbc-app", var.partner)))
  mssql_server_name                     = coalesce(var.mssql_server_name, lower(format("%s-rbc-sql", var.partner)))
  mssql_database_name                   = coalesce(var.mssql_database_name, lower(format("%s-rbc-db", var.partner)))
  mssql_private_endpoint_name           = coalesce(var.mssql_private_endpoint_name, format("%s-rbc-mssql-pe", var.partner))
  form_recognizer_name                  = coalesce(var.form_recognizer_name, lower(format("%s-rbc-cs-fr", var.partner)))
  form_recognizer_private_endpoint_name = coalesce(var.form_recognizer_private_endpoint_name, lower(format("%s-rbc-cs-fr-pe", var.partner)))
  app_gateway_name                      = coalesce(var.app_gateway_name, lower(format("%s-rbc-app-gw", var.partner)))
  private_link_configuration_name       = coalesce(var.app_gateway_private_link_configuration_name, lower(format("%s-rbc-app-gw-plc", var.partner)))
  log_analytics_workspace_name          = coalesce(var.log_analytics_workspace_name, lower(format("%s-rbc-law", var.partner)))
  application_insights_name             = coalesce(var.application_insights_name, lower(format("%s-rbc-mon-insights", var.partner)))
  virtual_network_name                  = coalesce(var.virtual_network_name, lower(format("%s-rbc-vnet", var.partner)))
  openai_account_name                   = coalesce(var.openai_account_name, lower(format("%s-rbc-cs-oai", var.partner)))
  openai_llm_deployment_name            = coalesce(var.openai_llm_deployment_name, lower(format("%s-rbc-oai-llm", var.partner)))
  openai_private_endpoint_name          = coalesce(var.openai_private_endpoint_name, lower(format("%s-rbc-cs-oai-pe", var.partner)))
  redis_cache_name                      = coalesce(var.redis_cache_name, lower(format("%s-rbc-redis", var.partner)))
  redis_private_endpoint_name           = coalesce(var.redis_private_endpoint_name, lower(format("%s-rbc-redis-pe", var.partner)))
  research_app_name                     = coalesce(var.research_app_name, lower(format("%s-rbc-research", var.partner)))
  research_storage_account_name         = coalesce(var.research_storage_account_name, lower(replace(format("%srbcdata", var.partner), "-", "")))
  firewall_name                         = coalesce(var.firewall_name, lower(format("%s-rbc-fw", var.partner)))
}
