...
variable "container_app_env_private_endpoint_name" {
  type        = string
  default     = ""
  description = "Name of the Container App Environment Private Endpoint resource."
}
...
locals {
  ...
  container_app_env_private_endpoint_name = coalesce(var.container_app_env_private_endpoint_name, lower(format("%s-env-pe", local.name_prefix)))
