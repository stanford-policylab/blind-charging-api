...
class MaskedDefendant(BaseModel):
    defendantId: str
    alias: str
...
class Documents(RootModel[Union[DocumentLink, DocumentText]]):
    root: Union[DocumentLink, DocumentText] = Field(..., discriminator='attachmentType')


class RedactionRequest(BaseModel):
    jurisdictionId: str
    caseId: str
    defendants: List[Defendant] = Field(..., min_length=1)
    documents: List[Documents] = Field(..., min_length=1)
    callbackUrl: AnyUrl
...
class FinalChargeOutcome(BaseModel):
    chargingDecision: ChargingDecision = Field(
        ..., description='The final charging decision.', examples=['CHARGE']
    )
    chargingDecisionExplanation: Optional[str] = Field(
        None, examples=['The defendant was caught on camera with the stolen goods.']
    )


class ChargingDecision1(Enum):
...
class BlindChargeOutcome(BaseModel):
    chargingDecision: ChargingDecision1 = Field(..., examples=['CHARGE_LIKELY'])
...
class BlindReviewInfo(BaseModel):
    ...
    maskedDefendants: List[MaskedDefendant]
